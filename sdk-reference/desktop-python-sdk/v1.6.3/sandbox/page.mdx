...
- `api_key`: NanoVM API Key to use for authentication, defaults to `NANOVM_API_KEY` environment variable
- `domain`: NanoVM domain to use for authentication, defaults to `NANOVM_DOMAIN` environment variable
- `debug`: If True, the sandbox will be created in debug mode, defaults to `NANOVM_DEBUG` environment variable
- `sandbox_id`: Sandbox ID to connect to, defaults to `NANOVM_SANDBOX_ID` environment variable
